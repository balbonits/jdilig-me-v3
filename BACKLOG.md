...
## 1. Add games to projects

Surface the AI-assisted browser games as their own project entries (currently only `squanto` and `jdilig-me` are listed).

**Scope**
- Add 1–3 game entries to `src/data/projects.ts` with `kind: 'GAME'`. The kind-aware live-link label already maps GAME → "Play".
- Capture gameplay screenshots via Playwright and add a `gallery` per game.
- Decide whether to host games here under a subdomain (e.g. `orbit-drift.jdilig.me`) or link out to wherever they live.
- If embedding gameplay, restore the browser-chrome iframe block in `src/routes/ProjectDetail.tsx` (currently superseded by `<ProjectGallery>` when a gallery exists).

**Notes**
- Source repos for the games aren't in this project yet — find them, list URLs, and decide which are public-shareable.
- Pre-existing dummy entries (Orbit Drift, Baseline, etc.) were stripped in commit `ec44cfb`; that commit can be referenced for shape if useful.

---

## 2. Lighthouse score on the jdilig.me project page
